  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      context 'defaults' do
        ...
        it { is_expected.to contain_file('/opt/crowd/atlassian-crowd-3.4.3-standalone') }
        it { is_expected.to contain_file('/var/local/crowd') }

        it { is_expected.to contain_staging__file('atlassian-crowd-3.4.3.tar.gz').with({
          :source => 'https://www.atlassian.com/software/crowd/downloads/binary/atlassian-crowd-3.4.3.tar.gz',
        })}

        it { is_expected.to contain_staging__extract('atlassian-crowd-3.4.3.tar.gz').with({
          :target  => '/opt/crowd/atlassian-crowd-3.4.3-standalone',
          :creates => '/opt/crowd/atlassian-crowd-3.4.3-standalone/apache-tomcat',
          :user    => 'crowd',
          :group   => 'crowd',
        })}
        ...
        it { is_expected.to contain_staging__file('jdbc driver').with({
          ...
          :target => '/opt/crowd/atlassian-crowd-3.4.3-standalone/apache-tomcat/lib/mysql-connector-java-5.1.36.jar',
        })}

        it { is_expected.to contain_exec('chown_/opt/crowd/atlassian-crowd-3.4.3-standalone').with({
          :command => 'chown -R crowd:crowd /opt/crowd/atlassian-crowd-3.4.3-standalone',
        })}
      end
    end
    ...
    context 'update crowd' do
      ...
      let :pre_condition do
        "class { 'crowd': version => '3.4.3' }"
      end
      it { is_expected.to contain_notify('Updating Crowd from version 2.11.1 to 3.4.3') }
    ...
    context 'custom parameters' do
      ...
      describe 'installdir' do
        ...
        it { is_expected.to contain_file('/svr/crowd/atlassian-crowd-3.4.3-standalone') }
        it { is_expected.to contain_staging__extract('atlassian-crowd-3.4.3.tar.gz').with({
          :target => '/svr/crowd/atlassian-crowd-3.4.3-standalone',
          :creates => '/svr/crowd/atlassian-crowd-3.4.3-standalone/apache-tomcat',
        })}
      end
      ...
      describe 'download_url using dns' do
        ...
        it { is_expected.to contain_staging__file('atlassian-crowd-3.4.3.tar.gz').with({
          :source => 'http://192.168.0.0.1:8080/atlassian-crowd-3.4.3.tar.gz',
        })}
      end
