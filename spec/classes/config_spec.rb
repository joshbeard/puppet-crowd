    ...
    context "on #{os}" do
      let(:facts) { facts }
        context 'defaults' do
          ...
          it { is_expected.to contain_augeas('/opt/crowd/atlassian-crowd-3.0.1-standalone/apache-tomcat/conf/server.xml').with({
            :changes => [
          ...
          it { is_expected.to contain_file('/opt/crowd/atlassian-crowd-3.0.1-standalone/apache-tomcat/bin/setenv.sh').with({
          ...
          it { is_expected.to contain_file('/opt/crowd/atlassian-crowd-3.0.1-standalone/apache-tomcat/bin/setenv.sh').with({
            :content => /CATALINA_PID="\/opt\/crowd\/atlassian-crowd-3.0.1-standalone\/apache-tomcat\/work\/crowd.pid"/,
          })}


          it { is_expected.to contain_file('/opt/crowd/atlassian-crowd-3.0.1-standalone/crowd-webapp/WEB-INF/classes/crowd-init.properties').with({
            :content => /crowd\.home=\/var\/local\/crowd/,
          })}

          it { is_expected.to contain_file('/opt/crowd/atlassian-crowd-3.0.1-standalone/apache-tomcat/conf/Catalina/localhost/openidserver.xml').with({
          ...
          it { is_expected.to contain_file('/opt/crowd/atlassian-crowd-3.0.1-standalone/crowd-openidserver-webapp/WEB-INF/classes/jdbc.properties').with({
        ...
        context 'custom parameter' do
          ...
          describe 'postgres' do
            ...
            it { is_expected.not_to contain_file('/opt/crowd/atlassian-crowd-3.0.1-standalone/apache-tomcat/conf/Catalina/localhost/openidserver.xml').with({
              :content => /validationQuery="Select 1"/
            })}
